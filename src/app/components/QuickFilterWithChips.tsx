...
interface QuickFilterWithChipsProps {
...
export function QuickFilterWithChips({
  categories,
  selectedIds,
  onApply,
  triggerStyle = "text",
}: QuickFilterWithChipsProps) {
  ...
  const getAllChildIds = (option: CategoryOption): string[] => {
    const ids = [option.id];
    if (option.children) {
  ...
  const getCategoryAllIds = (category: FilterCategory): string[] => {
    const ids: string[] = [];
    category.options.forEach(option => {
      ids.push(...getAllChildIds(option));
    });
    return ids;
  };

  const handleCheckboxChange = (option: CategoryOption, checked: boolean) => {
  ...
  const handleChipToggle = (categoryId: string) => {
    const isSelected = selectedChips.includes(categoryId);
    
    if (isSelected) {
      setSelectedChips(selectedChips.filter(id => id !== categoryId));
      const category = categories.find(c => c.id === categoryId);
      if (category) {
        const categoryIds = getCategoryAllIds(category);
        setTempSelected(tempSelected.filter(id => !categoryIds.includes(id)));
      }
    } else {
      setSelectedChips([...selectedChips, categoryId]);
      ...
      if (category) {
        const categoryIds = getCategoryAllIds(category);
        setTempSelected([...new Set([...tempSelected, ...categoryIds])]);
      }
    }
  };

  const isParentChecked = (option: CategoryOption): boolean => {
  ...
  const isParentIndeterminate = (option: CategoryOption): boolean => {
    if (!option.children || option.children.length === 0) {
      return false;
    }
  ...
  const renderOption = (option: CategoryOption, depth: number = 0) => {
  ...
  return (
    <div style={{ position: "relative", display: "inline-block" }}>
      ...
      {triggerStyle === "text" ? (
        ...
        >
          <span style={{ textDecoration: "underline", color: "inherit" }}>Advanced Filter</span>
          {selectedCount > 0 && (
            <span
              style={{
                display: "inline-flex",
        ...
        >
          <span style={{ color: "var(--partnerhome-text-color-primary)" }}>Advanced Filter</span>
          {selectedCount > 0 && (
            <span
              style={{
                display: "inline-flex",
          ...
          >
            {categories.map(category => (
              <Chip
                ...
                dismissible={false}
                onClick={() => handleChipToggle(category.id)}
              />
            ))}
          </div>
        }
      ...
      >
        {categories.map((category, index) => (
          <div key={category.id} style={{ width: "100%" }}>
            ...
            {/* Category Options */}
            {category.options.map(option => renderOption(option))}

            {/* Category Divider */}
            {index < categories.length - 1 && <DropdownMenuDivider />}
          </div>
        ))}
      </DropdownMenu>
    </div>
